...
const PropertyType = () => {
  const { data, loading, err } = useFetch(
    "https://full-stack-hotel-booking-app-eosin.vercel.app/api/hotels/countByType"
  );
  const images = [
  ...
  return (
    <div className="my-6">
      <div className="container mx-auto px-6">
        <h3 className="text-2xl font-bold md:mb-6">Browse by property type</h3>
          ...
          {data.length > 1 ? (
            images.map((img, i) => {
              return (
                <div
                  key={i}
                  className="pitem md:w-[23%] md:h-[200px] cursor-pointer mt-6 md:mt-0 "
                >
