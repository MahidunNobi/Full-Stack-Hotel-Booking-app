  ...
  const fetchData = async () => {
    try {
      const res = await axios.get(
        "https://full-stack-hotel-booking-app-eosin.vercel.app/api/hotels/find/64e45197928fcdbde0ba0c63/rooms"
      );
      setRooms(res.data);
    } catch (error) {
      console.log(error);
      ...
      await Promise.all(
        selectedRooms.map((roomId) => {
          const res = axios.patch(
            `https://full-stack-hotel-booking-app-eosin.vercel.app/api/rooms/availability/${roomId}`,
            { dates: allDates }
          );
      ...
      <div className="mainContainer h-[80vh] flex justify-center items-center">
        <div className="bg-white w-[350px] p-6 rounded-lg">
          ...
          <div className="rooms">
            {rooms.map((room, i) => {
              return (
                <div key={room._id} className="room flex justify-between mb-5">
                  <div className="left w-[60%]">
                    <span className="text-lg"> {room.title}</span>
                    <br />
                    <span className="desc text-gray-500">
                      {" "}
                      {room.desc}
                    </span>{" "}
                    <br />
                    <span className=" MaxPeople text-sm">
                      {" "}
                  ...
                  <div className="right w-[35%] ml-3 flex justify-between flex-wrap space-y-0">
                    {room.roomNumbers.map((num, i) => {
                      return (
                        <div
                          key={num._id}
                          className="num text-gray-500 flex flex-col items-center mx-1"
                        >
                          <label htmlFor="101" className=" text-xs">
                            {" "}
                            {num.number}{" "}
                          </label>
                          <input
